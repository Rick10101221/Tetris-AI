...
def clear_rows(grid, locked):
    ...
    # Counts how many rows need to be deleted
    inc = 0
    for i in range(len(grid) - 1, -1, -1):
        # Grabs a row from grid (since grid is a list of lists, grid[i] will return the row)
        row = grid[i]
        # If there is no black square in the current row
        if (0,0,0) not in row:
            inc += 1
            ind = i
            # For every square in the row, try to delete the piece in locked
            # TODO LOCKED DATA TYPE
